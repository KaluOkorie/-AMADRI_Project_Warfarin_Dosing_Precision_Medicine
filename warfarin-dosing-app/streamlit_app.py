...
try:
    ...
    st.error("❌ Model file 'final_warfarin_model_xgboost.pkl' not found. Please ensure it is in the app directory.")
    st.stop()
...
if calculate_button:
    ...
    if result["error"]:
        ...
        st.write(f"Start with **{result['prediction'] * 0.5:.2f} mg** daily for 2 days, then adjust by 0.5–1 mg based on INR response.")
        st.write(f"**Target INR:** 2.0–3.0 (standard) or 2.5–3.5 (mechanical valves)")
    ...
    st.info("👈 **Please fill in the patient parameters on the sidebar and click 'Calculate Warfarin Dose'.**")
